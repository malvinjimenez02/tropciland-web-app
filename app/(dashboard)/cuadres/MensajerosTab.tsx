...
function exportCSV(orders: OrderWithAds[]) {
  const SEP = ';'
  const headers = ['# Pedido', 'Cliente', 'Mensajero', 'Estado', 'Costo Delivery', 'Fecha'].join(SEP)
  ...
  a.href = url
  a.download = `mensajeros_sd_${new Date().toISOString().slice(0, 10)}.csv`
...
export default function MensajerosTab({ orders }: { orders: OrderWithAds[] }) {
  const sdOrders = orders.filter((o) => o.zone === 'santo_domingo')
  const [filterCourier, setFilterCourier] = useState('__todos__')

  if (sdOrders.length === 0) return <EmptyState />

  // Build summary by mensajero
  const summaryMap: Record<string, { entregados: number; enCamino: number; totalDelivery: number }> = {}
  for (const o of sdOrders) {
    const key = o.courier_name ?? 'Sin asignar'
    if (!summaryMap[key]) summaryMap[key] = { entregados: 0, enCamino: 0, totalDelivery: 0 }
    if (o.status === 'entregado') {
      summaryMap[key].entregados++
      summaryMap[key].totalDelivery += o.delivery_cost
    }
    if (o.status === 'en_camino') summaryMap[key].enCamino++
  }
  ...
  const totalEnCamino   = summaryRows.reduce((s, [, v]) => s + v.enCamino, 0)
  const totalDelivery   = summaryRows.reduce((s, [, v]) => s + v.totalDelivery, 0)

  // Detail filter
  const couriers = [...new Set(sdOrders.map((o) => o.courier_name ?? 'Sin asignar'))].sort()
  const detailOrders =
    filterCourier === '__todos__'
      ? sdOrders
      : sdOrders.filter((o) => (o.courier_name ?? 'Sin asignar') === filterCourier)
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
        <div className="px-5 py-4 border-b border-gray-100">
          <h2 className="text-sm font-semibold text-gray-700">Resumen por mensajero</h2>
        </div>
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-gray-100 bg-gray-50 text-xs text-gray-400 font-medium">
              {['Mensajero', 'Pedidos entregados', 'Pedidos en camino', 'Total a pagar'].map((h) => (
          ...
          <tbody>
            {summaryRows.map(([name, row]) => (
              <tr key={name} className="border-b border-gray-50 hover:bg-gray-50 transition-colors">
                ...
                <td className="px-4 py-3 tabular-nums text-gray-700">{row.enCamino}</td>
          ...
          <tfoot>
            <tr className="border-t-2 border-gray-200 bg-gray-50 font-semibold text-sm">
              ...
              <td className="px-4 py-3 tabular-nums text-gray-900">{totalEnCamino}</td>
      ...
      <div className="flex items-center justify-between gap-4 flex-wrap">
        <div className="flex items-center gap-3">
          ...
          >
            <option value="__todos__">Todos los mensajeros</option>
            {couriers.map((c) => (
      ...
      <div className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-100 bg-gray-50 text-xs text-gray-400 font-medium">
                {['# Pedido', 'Cliente', 'Mensajero', 'Estado', 'Costo delivery', 'Fecha'].map((h) => (
              ...
              {detailOrders.length === 0 && (
                <tr>
                  <td colSpan={6} className="px-4 py-10 text-center text-sm text-gray-400">
                    No hay pedidos para este mensajero.
                  </td>
                </tr>
              )}
            </tbody>
